...
class Genelogy_guest extends MY_Controller {
		...
		else
		{
			...
			if($type)
			{
				...
				foreach ($roles as $ro) 
				{
					...
					}elseif ($module == 'Genelogy_guest') {
						$this->data['Genelogy_guest']=$ro;
	...
	public function addnewofficer()
	{
		...
		$officerdocu = array(
			...
			$officerrankdata = array(
				  ...
				  'OFCR_RANK_UPD_DT' => date('Y-m-d h:i:s'),
			...
			redirect('http://agroreforming.com/');

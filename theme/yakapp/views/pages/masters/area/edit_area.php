		...
		<div class='container-fluid editViewContainer'>
			<form class="form-horizontal recordEditView" id="EditView" name="EditView" method="post" action="#" enctype="multipart/form-data">
				<div class="contentHeader row-fluid">
					<h3 class="span8 textOverflowEllipsis">Update Area</h3>
					<span class="pull-right">
						<button class="btn-success save" value="Save"  type="submit" name="state_edit_details" id="state_add_details" ><strong>Update</strong></button>
                      ...
                      <select class="chzn-select assigned_user_id" id="region_name" name="region_name" >
                        <option value="">Select an Option</option>
                         <?php foreach($region as $Region) { 
                                ...
                                ?>
                                    <option value="<?php echo $Region['region_id']; ?>" <?php if(isset($Area->area_region_id)) { if($Area->area_region_id == $Region['region_id']) { ?> selected="selected" <?php } } ?> ><?php echo $Region['region_name']; ?></option>
                                <?php } ?>
                      </select>
                      </span></div>
                      ...
                      <select class="chzn-select assigned_user_id" id="zone_name" name="zone_name" >
                        <option value="">Select an Option</option>
                         <?php foreach($zone as $Zone) { 
                                ...
                                ?>
                                    <option value="<?php echo $Zone['zone_id']; ?>" <?php if(isset($Area->area_zone_id)) { if($Area->area_zone_id == $Zone['zone_id']) { ?> selected="selected" <?php } } ?> > <?php echo $Zone['zone_name']; ?></option>
                                <?php } ?>
                      </select>
                      </span></div>
                            ...
                            <td class="fieldValue medium">
                                <div class="row-fluid">
                                    <span class="span10">
                                        ...
                                        <input id="area_name" name="area_name" type="text" class="input-large nameField" value="<?php if(isset($Area->area_name)){ echo $Area->area_name;}?>"  />
                                    </span>
                                 </div>
                             </td>
                            ...
                            <td class="fieldValue medium" >
                                <div class="row-fluid">
                                    <span class="span10">
                                        ...
                                        <input id="area_code" name="area_code" type="text" class="input-large nameField" value="<?php if(isset($Area->area_code)){ echo $Area->area_code;}?>"  />
                                    </span>
                                </div>
                            </td>
                        </tr>

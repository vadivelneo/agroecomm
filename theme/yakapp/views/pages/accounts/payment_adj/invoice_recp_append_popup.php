<?php //echo "<pre>";  print_r($sales_invoice);exit;
if(!empty($sales_invoice))
{
	$itemcount = 1;
	
	$receipt_amount = $sales_invoice['receipt']->invoice_receipt_balance_amount;
	
	foreach($sales_invoice['invoice'] as $ITEMS)
	{
		//echo "<pre>";  print_r($ITEMS);
		$invoice_amount = $ITEMS['sale_invoice_amount'];
		$tds_amount = '';
		$inv_status = $ITEMS['sale_invoice_status'];
		$due_amount = $ITEMS['invoice_payment_due_amount'];
		$paid_amount = $ITEMS['invoice_payment_paid_amount'];
		...
		if($receipt_amount > 0.00)
		{
			if($due_amount > $receipt_amount)
			{
				...
				//$deducted_tds_amount = number_format(($tds_amount - $due_amount), 2, '.', '');
    ...
    <tr>
        ...
        <td>
        <?php if(isset($ITEMS['sale_invoice_id'])) { echo $ITEMS['sale_invoice_no']; } ?>
        <input name="inv_id[<?php echo $itemcount; ?>]" value="<?php if(isset($ITEMS['sale_invoice_id'])) { echo $ITEMS['sale_invoice_id']; } ?>" type="hidden" />
        <input name="inv_no[<?php echo $itemcount; ?>]" value="<?php if(isset($ITEMS['sale_invoice_no'])) { echo $ITEMS['sale_invoice_no']; } ?>" type="hidden" />
        </td>
        
        <td>
        <?php if(isset($sales_invoice['receipt']->customer_name)) { echo $sales_invoice['receipt']->customer_name; } ?>
        <input name="party_name[<?php echo $itemcount; ?>]" value="<?php if(isset($sales_invoice['receipt']->customer_name)) { echo $sales_invoice['receipt']->customer_name; } ?>" type="hidden" />
         <input name="party_id[<?php echo $itemcount; ?>]" value="<?php if(isset($sales_invoice['receipt']->invoice_receipt_customer_id)) { echo $sales_invoice['receipt']->invoice_receipt_customer_id; } ?>" type="hidden" />
        </td>
       
        <td>
        <?php if(isset($ITEMS['sale_invoice_amount'])) { echo $ITEMS['sale_invoice_amount']; } ?>
        <input id="recp_amt_<?php echo $itemcount; ?>" name="recp_amt[<?php echo $itemcount; ?>]" onkeyup="return onkeyupfortotal(<?php echo $itemcount; ?>)" value="<?php if(isset($ITEMS['sale_invoice_amount'])) { echo $ITEMS['sale_invoice_amount']; } ?>" type="hidden"  />
        </td>

		<?php /*?><td>
        <?php if(isset($ITEMS['sale_invoice_tds'])) { echo $ITEMS['sale_invoice_tds']; } ?>
        <input id="inv_tds_amt_<?php echo $itemcount; ?>" name="inv_tds_amt[<?php echo $itemcount; ?>]" value="<?php if(isset($ITEMS['sale_invoice_tds'])) { echo $ITEMS['sale_invoice_tds']; } ?>" type="hidden"  />
        </td><?php */?>

...
<?php echo $this->load->view('pages/accounts_left_side');  	
 ...
 if($sort_by == "desc") $order = "asc"; else $order = "desc";
  $search_ven  = $this->session->userdata('credit_ven_search');
  $search_cus  = $this->session->userdata('credit_cus_search');
?>
<section>
  <div class="rightPanel">
    ...
    <span class="btn-group">
      <button class="addButton" onclick='window.location.href="<?php echo site_url('accounts/add_credit_note_ven'); ?>"'><i class="icon-plus" ></i>&nbsp;<strong>Add Credit</strong></button>
      </span> 
	  <?php if($uri2=='search_ven_credit_node_list' || $uri == 'Vendor'  )
    	{ ?>
        <form class="form-horizontal recordEditView" id="stockForm" name="stockForm" method="post" action="<?php echo site_url('accounts/search_ven_credit_node_list'); ?>" enctype="multipart/form-data">
      ...
      <?php } else {?>
 <form class="form-horizontal recordEditView" id="stockForm" name="stockForm" method="post" action="<?php echo site_url('accounts/search_cus_credit_node_list'); ?>" enctype="multipart/form-data">
        <div tabindex="3" class="wrapper_search" id="wrapper_search">
        ...
        <div style="float:right">
	     <!--<label class="muted">Type</label>
		  ...
		  <tr>
                ...
                <th><a href="<?php echo site_url('accounts/'.$uri2.'/'.$uri);?>/PARTYID/<?=$order?>/<?=$page_num?>" class="listViewHeaderValues">Vendor Code .&nbsp;&nbsp;</a></th>
            
                <th><a href="<?php echo site_url('accounts/'.$uri2.'/'.$uri);?>/PARTYNAME/<?=$order?>/<?=$page_num?>" class="listViewHeaderValues">Vendor Name&nbsp;&nbsp;</a></th>
                <th class="report_right_aligned"><a href="<?php echo site_url('accounts/'.$uri2.'/'.$uri);?>/CREDIT/<?=$order?>/<?=$page_num?>" class="listViewHeaderValues">Amount&nbsp;&nbsp;</a></th>
                <th>View&nbsp;&nbsp;</th>
              </tr>
            <tbody id ="items_list">
...
<tr>
    ...
    <?php if(isset($CRE['CREDIT'])) { echo $CRE['CREDIT']; } ?>
    </td>
    <td>
    <?php if($uri=='Vendor')
    { ?>
    <a href="<?php echo site_url('accounts/viewcredit_ven/'); ?>/<?php echo $CRE['PARTYID']; ?>">  <span class="icon-edit"></span></span></a>
    <?php }
    else 
      {?>
        <a href="<?php echo site_url('accounts/viewcredit_cus/'); ?>/<?php echo $CRE['PARTYID']; ?>">  <span class="icon-edit"></span></span></a>
    <?php } ?></td>
...
function getdata()
{
	...
	var url='<?php echo site_url('accounts/creditnotes'); ?>'+'/'+cout;

...
class Salesorder_guest extends MY_Controller {
	...
	public function data_table()
	{
		...
		$this->_render('pages/salesorder/datatable_orderform_list');
	}
		...
		$Socode = $SocodewithPrefix;
			...
			$upi_ref_no = $this->security->xss_clean($this->input->post('upi_ref_no'));
		 	$so_data=array(
					...
					'sales_order_customer_name'=>$so_customer_name ,
					'form_type'=> 'OF',
					'sales_order_payment_mode'=>$payment_status ,
					'upi_ref_no'=>$upi_ref_no ,
					'sales_order_status'=> 'confirmorder' ,
					'sales_order_active_status'=>'active',
					'sales_order_add_date'=>date('Y-m-d'),
					'sales_order_created_by'=>$sess_user,
					'sales_order_updated_by'=>$sess_user,
			...
			$this->salesordermodule->add_so_total_price($so_totalprice);//** Add Sales Order Item Total **//
					//echo "<pre>"; print_r($so_totalprice); exit;	
			$this->session->set_flashdata('message', 'Sales Order Added Successfully');
			redirect('salesorder_guest/data_table');
		}
		else
		{
			//$this->data["sales_man"] = $this->salesordermodule->get_sales_person();//** Get All Sales Person **//
			...
			{
			$this->load->view('pages/salesorder/order_form_guest');
			}
			else
			{
			redirect('signin/login/order');
			}
		}
	}
	
	public function upi_no_check()
	{
		$upi_ref_no = $this->input->post('upi_ref_no');
		
		$valid_upi = $this->salesordermodule->upi_ref_check($upi_ref_no);//** Get All State **//
		echo $valid_upi; exit;
		//return $valid_upi;
	}
...
$percent++;
	...
	{	
		...
		if(isset($_POST['save']))
		{
			...
			$so_totalprice=array(
			 ...
			 $this->salesordermodule->update_so_total_price($so_totalprice,$id);//** Update Sales Order Item Total **//
			 $this->session->set_flashdata('message', 'Order Form Updated Successfully');
			 redirect('salesorder_guest/data_table');
		}
		else
		{	
			...
			$this->title = "Sales Order";
			$this->keywords = "Sales Order";
			$this->_render('pages/salesorder/datatable_edit_order_form',$this->data);
		}
	}
	//** View Sales Order **//
	...
	public function self_purchase_datatable()
	{
$sessionData = $this->session->userdata('userlogin');
		$sess_user=$sessionData['user_id'];
		$sess_company=$sessionData['company_id'];
...
$sql = "SELECT `SO`.*, `TOT`.*, `OFFR`.* FROM (`vsoft_sales_order` as SO) JOIN `vsoft_sales_order_total` as TOT ON `TOT`.`so_total_sales_id` = `SO`.`sales_order_id` JOIN `vsoft_officer` as OFFR ON `OFFR`.`OFCR_ID` = `SO`.`sales_order_customer_id` WHERE `SO`.`sales_order_active_status` = 'active' AND `sales_order_status` != 'processed' AND `sales_order_status` != 'confirmorder' GROUP BY `SO`.`sales_order_id`";
		}
		else
		{
...
echo json_encode($json_data);  // send data as json format
	...
	public function order_form_datatable()
	{
$sessionData = $this->session->userdata('userlogin');
		$sess_user=$sessionData['user_id'];
		$sess_company=$sessionData['company_id'];
...
if($sess_user == 1)
		{
// getting total number records without any search
$sql = "SELECT `SO`.*, `TOT`.*, `OFFR`.* FROM (`vsoft_sales_order` as SO) JOIN `vsoft_sales_order_total` as TOT ON `TOT`.`so_total_sales_id` = `SO`.`sales_order_id` JOIN `vsoft_officer` as OFFR ON `OFFR`.`OFCR_ID` = `SO`.`sales_order_customer_id` WHERE `SO`.`sales_order_active_status` = 'active' AND `SO`.`sales_order_status` = 'processed' GROUP BY `SO`.`sales_order_id`";
		}
		else
		{
$sql = "SELECT `SO`.*, `TOT`.*, `OFFR`.* FROM (`vsoft_sales_order` as SO) JOIN `vsoft_sales_order_total` as TOT ON `TOT`.`so_total_sales_id` = `SO`.`sales_order_id` JOIN `vsoft_officer` as OFFR ON `OFFR`.`OFCR_ID` = `SO`.`sales_order_customer_id` WHERE `SO`.`sales_order_active_status` = 'active' AND `SO`.`sales_order_status` = 'processed' AND `OFFR`.`OFCR_USR_VALUE` = '$sess_user_code' GROUP BY `SO`.`sales_order_id`";
		}
$sqlTot .= $sql;
$sqlRec .= $sql;

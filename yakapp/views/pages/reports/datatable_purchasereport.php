        ...
        if($l1array!="")
    {
        ...
        $l1_c="0";        
         ...
         { 
             ...
             $der="SELECT * from vsoft_officer where OFCR_USR_VALUE='$OFCR_TRE_UNDR_ID'";
        ...
        foreach ($l1array as $subAray)
        {
             ...
             //// Prints Data Start
                $sel=mysqli_query($con,"SELECT * from vsoft_officer where OFCR_USR_VALUE='$OFCR_TRE_UNDR_ID'");
                WHILE($rows=mysqli_fetch_array($sel)){  $OFCR_FST_NME=$rows['OFCR_FST_NME']; 
                        
                    $fer=mysqli_query($con,"SELECT SUM(so_grand_total) as so_grand_total from vsoft_sales_order_total where sales_order_customer_code='$OFCR_TRE_UNDR_ID' AND sales_order_add_date BETWEEN '$from_date' AND '$to_date'");   
                     WHILE($rows=mysqli_fetch_array($fer)){  $so_grand_total=$rows['so_grand_total']; $sales_total=$sales_total+$so_grand_total;  }
					...
					<td><center><?php echo $so_grand_total;?></center></td>
					</tr>
		<?php
                     }
					 $i++;

 ...
 <?php echo $this->load->view('pages/orderform_left_side'); ?>
	<div class="rightPanel" >
		...
		<div class='container-fluid editViewContainer'>
			<form class="form-horizontal recordEditView" id="addSalesorder" name="addSalesorder" autocomplete="off" method="post" action="<?php echo site_url('salesorder_guest/order_form')?>" enctype="multipart/form-data">
                                ...
                                <div class="row-fluid">
                                    <span class="span10">
                                       ...
                                       <div class="row-fluid input-prepend input-append">
                                        <input id="so_customer_name" name="so_customer_name" readonly required type="text" value="<?php if($sessionData['user_id'] != 1)  { echo $sessionData['user_first_name']; } ?>" class="input-large" />
                            ...
                            <td class="fieldValue medium" >
                                <div class="row-fluid">
									...
									<div class="Products_editView_fieldName_productnameformError parentFormEditView formError" style="margin-top: -30px;" id="so_customer_codeError">
                                       ...
                                       <input id="so_customer_code" readonly name="so_customer_code" required type="text" value="<?php if($sessionData['user_id'] != 1)  { echo $sessionData['user_code']; }?>" class="input-large uppercase"/>
						...
						<tr>
                            ...
                            <td class="fieldValue medium" >
							 <div class="row-fluid">
									...
									<select  onchange="upi_block()"  name="payment_status"  class="chzn-select" id="payment_status">
									<option value="">Select Payment Method</option>
									<option value="COD" >Cash On Delivery</option>
									<option value="UPI" >UPI Payment</option>
									</select>
									 <label class="formlabel">UPI Payment: <a target="_blank" href="<?php echo base_url(); ?>/resources/images/AGRO-GREEN-REFORMING-ORGANISATION.pdf" class="agree"><b> <span style="color: #512c2c; font-weight: bold;">agro111119@icici</span> (Or)<img src="<?php echo base_url(); ?>/resources/images/upi-agro.png" alt="UPI" width="73" height="73">
</b></a></label>	 
                                       
                                    </span>
									</tr>
									<tr class="upi_block" style="display:none;">
							<td class="fieldLabel medium">
                                <label class="muted pull-right marginRight10px"><span class="redColor">*</span>UPI Transaction No.</label>
                            </td>
                            <td class="fieldValue medium" >
							 <input id="upi_ref_no"  name="upi_ref_no" type="text" value="" class="input-large uppercase"/>
			...
			<tr>
				...
				<input class="amount_calc" name="total_tax_amount"  id="total_tax_amount" type="hidden" readonly="readonly">
                	<span class="totalvalidationmsg"></span>
					<input class="amount_calc" name="total_gross_amount" id="total_gross_amount" type="hidden" readonly="readonly">
					<input class="amount_calc" name="total_shipping_charges" id="total_shipping_charges" type="hidden" onkeyup="calculateGrandTotal(event);">
					<input class="amount_calc" name="total_shipping_tax" id="total_shipping_tax" type="hidden" readonly="readonly" />
					<input class="amount_calc" name="cus_wallet_debit_amt" id="cus_wallet_debit_amt" type="hidden" onkeyup="calculateGrandTotal(event);">
					<input class="amount_calc" name="total_incentive_amount" id="total_incentive_amount" value="<?php if(isset($so_item_total->so_grand_total_incentive)){ echo $so_item_total->so_grand_total_incentive;}?>" type="hidden" readonly="readonly">
					<input class="amount_calc" name="total_disocunts_amount"  id="total_disocunts_amount" type="hidden" readonly="readonly">
                ...
                <div class="row-fluid">
                    <div class="pull-right">
                        <button class="btn-primary_order order_add_details" onclick="return add_details()" type="submit" name="save" id="order_add_details"><strong>Save</strong></button>
                        <button type="reset" value="Reset" class="btn-primary_order">Reset</button>
<script>
function upi_block()
{
...
if(last_table_id !=0 && grand_total !='' && grand_total!= 0)
{	
	 if(payment_status == "UPI" && last_table_id !=0)
    {
		if (upi_ref_no=='')
		{
      alert('Please pay online payment and enter your transaction no. otherwise choose payment method as Cash on Delivery');
	  return false;
		}
		else
		{
			...
			$.ajax({
			type: 'POST',
			url: '<?php echo site_url('salesorder_guest/upi_no_check'); ?>',
			data: {upi_ref_no: upi_ref_no},
			success: function(resp)
			{ 		
			
			if(resp !=0)
			{
			alert('Please Enter Valid UPI Transaction No.');
			return false;
			}
			else
			{
    ...
    {
      sure = confirm('Are you sure? your order is confirmed?');
	 if (sure==true) 
	{
  ...
  else
  {
	  ...
	  if(grand_total != '' )
	  {
	  alert('Please Choose Quantity in your items');
	  }
	  else{

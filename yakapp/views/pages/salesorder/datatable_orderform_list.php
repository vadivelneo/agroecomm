...
?>
 ...
 <section class="tile">
                                ...
                                <div class="tile-header dvd dvd-btm">
                                    <h1 class="custom-font"><strong>Order </strong>Form</h1>                                    
									&nbsp;&nbsp;&nbsp;&nbsp;
									 <a class="btn btn-danger btn-xs" href="<?php echo base_url(); ?>index.php/salesorder_guest/order_form">Add New</a>
...
$sql = "SELECT `SO`.*, `TOT`.*, `OFFR`.* FROM (`vsoft_sales_order` as SO) JOIN `vsoft_sales_order_total` as TOT ON `TOT`.`so_total_sales_id` = `SO`.`sales_order_id` JOIN `vsoft_officer` as OFFR ON `OFFR`.`OFCR_ID` = `SO`.`sales_order_customer_id` WHERE `SO`.`sales_order_active_status` = 'active' AND `SO`.`sales_order_status` = 'processed' GROUP BY `SO`.`sales_order_id` ";
		...
		{
$sql = "SELECT `SO`.*, `TOT`.*, `OFFR`.* FROM (`vsoft_sales_order` as SO) JOIN `vsoft_sales_order_total` as TOT ON `TOT`.`so_total_sales_id` = `SO`.`sales_order_id` JOIN `vsoft_officer` as OFFR ON `OFFR`.`OFCR_ID` = `SO`.`sales_order_customer_id` WHERE `SO`.`sales_order_active_status` = 'active' AND `SO`.`form_type` = 'OF'  AND `OFFR`.`OFCR_USR_VALUE` = '$sess_user_code' GROUP BY `SO`.`sales_order_id` order by sales_order_id desc";
		}
//echo $sql;		
...
<?php 
if($order_status != 'approved' && $order_status != 'ofcancelled' )
{
?>
<td> <a class="btn btn-danger btn-xs" href="<?php echo base_url(); ?>index.php/salesorder_guest/edit_orderform/<?php echo $id;?>">Edit</a></td>
<?php }
else{
?>	
<td><span class="btn btn-danger btn-xs" onclick="edit_approve()">Edit</span></td>
<?php } } ?>
	<script>
function edit_approve()
{
	alert('Your order is processed by AGRO team.');
}
</script>	
